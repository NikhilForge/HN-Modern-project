...
const BASE_URL = process.env.NEXT_PUBLIC_HACKER_NEWS_API_BASE || 'https://hacker-news.firebaseio.com/v0';
 ...
 * @returns Array of stories with details
 */
 ...
 * @returns Array of comment details
 */
export async function getComments(commentIds: number[]): Promise<HNStory[]> {
  try {
    ...
    return comments.filter(comment => comment && !comment.deleted);
  } catch (error) {
...
export function categorizeStory(story: HNStory): Category {
  ...
  if (title.includes('python') || title.includes('javascript') || 
      title.includes('rust') || title.includes('go ') || title.includes('programming') ||
      title.includes('code') || title.includes('developer')) {
    return 'Programming';
  }
  
  // Startup keywords
  if (title.includes('startup') || title.includes('founder') || 
      title.includes('vc ') || title.includes('funding') || title.includes('yc ')) {
    return 'Startups';
  }
  
  // Open Source keywords
  if (title.includes('open source') || title.includes('github') || 
      title.includes('oss') || url.includes('github.com')) {
    return 'Open Source';
  }
  
  // Security keywords
  if (title.includes('security') || title.includes('vulnerability') || 
      title.includes('breach') || title.includes('hack') || title.includes('crypto')) {
    return 'Security';
  }
